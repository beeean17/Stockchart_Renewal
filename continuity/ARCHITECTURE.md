...
**핵심 보안 규칙**:
- 주식 데이터: 모두 읽기 가능, Cloud Function만 쓰기
- 사용자 가로선: 본인만 접근 (인증 필수)
- 메타데이터: 모두 읽기 가능

**보안 규칙 구조**:
```
stocks: allow read: true, write: false (Cloud Function만)
users/{userId}/lines: allow read/write: if auth.uid == userId
metadata: allow read: true, write: false
```
...
**설정 방법**: Firebase Functions config 사용
