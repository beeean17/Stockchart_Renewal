...
```
앱 시작:
- 종목 리스트: 1 read (모든 종목 정보)
- 선택 종목 정보 (recent 포함): 1 read

차트 데이터:
- 최근 90일: 0 read (종목 문서에 포함됨)
- 추가 6개월: 6 reads (월별 문서 6개)
- 추가 1년: 12 reads (월별 문서 12개)

가로선:
- 로그인 시 가로선 로드: 1 read (쿼리)

종목 전환:
- 새 종목 선택: 1 read (recent 포함)
- 추가 히스토리 필요시: +월별 문서

일일 평균 (차트 5종목, 1년 히스토리):
- 종목 문서: 5 reads
- 월별 문서 (1년): 5 × 12 = 60 reads
- 가로선: 5 reads
...
증분 동기화만:
- 종목 문서 (최신 확인): 5 reads
- 당일 월 문서 (업데이트 확인): 1 read

2명 총: ~20 reads/일
할당량 대비: 0.04%
```

**할당량 대비**: 50,000의 **0.04~0.28%**

**결론**: ✅ 압도적으로 여유로움 (월별 구조로 인한 효율)
...
### 시나리오 1: 현재 계획 (2명, 월별 구조)
| 리소스 | 사용량 | 할당량 대비 | 비용 |
|---------|---------|-------------|------|
| Firestore 읽기 | 600/월 (20/일) | 0.04% | $0 |
| Firestore 쓰기 | 2,070/월 | 0.35% | $0 |
| Cloud Functions | 30회/월 | 0.0015% | $0 |
| **총 비용** | - | - | **$0** |

### 시나리오 2: 활성 사용 (2명, 매일 여러 종목)
| 리소스 | 사용량 | 할당량 대비 | 비용 |
|---------|---------|-------------|------|
| Firestore 읽기 | 4,200/월 (140/일) | 0.28% | $0 |
...
| 리소스 | 사용량 | 할당량 대비 | 비용 |
|---------|---------|-------------|------|
| Firestore 읽기 | 21,000/월 (700/일) | 1.4% | $0 |
| Firestore 쓰기 | 5,000/월 | 0.83% | $0 |
| Cloud Functions | 30회/월 | 0.0015% | $0 |
| **총 비용** | - | - | **$0** |

### 시나리오 4: 일별 구조 비교 (2명, 참고용)
| 리소스 | 사용량 | 할당량 대비 | 비용 |
|---------|---------|-------------|------|
| Firestore 읽기 | 37,500/월 (1,250/일) | 2.5% | $0 |
| Firestore 쓰기 | 15,000/월 | 2.5% | $0 |
| **비고** | 월별 구조 대비 18배 | - | - |

> **참고**: 월별 구조로 인해 읽기가 약 95% 감소. 동일 사용 패턴에서 훨씬 효율적.
